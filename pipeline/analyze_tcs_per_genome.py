...
def initialize(argv):
	global INPUT_FILE1, INPUT_FILE2, INPUT_FILE3, OUTPUT_FILE1, OUTPUT_FILE2, OUTPUT_FILE3, OUTPUT_FILE4
	try:
		opts, args = getopt.getopt(argv[1:],"hi:s:t:f:g:k:l:",["help", "ifile=", "sfile=", "tfile=", "ffile=", "gfile=", "kfile=", "lfile="])
		if len(opts) == 0:
...
def writeToFile(dataDict, Genome_id, outputFile):
	if Genome_id != "Genome_id":
		with open (outputFile, "a") as oFile:
			for element, count in dataDict.items():
				...
				oFile.write("\t".join([Genome_id, element, str(count), str(normed_bysize), str(normed_byprotcount)]) + "\n")
